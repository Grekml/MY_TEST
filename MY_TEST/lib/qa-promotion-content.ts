...
const totalTestedTasksFromCards = PROJECTS.reduce(
  (acc, project) =>
    acc + parseMetricFromCards(project.cardMetrics, "Задач протестировано"),
  0,
);
...
export const skillTracks: SkillTrack[] = [
  ...
  },
  {
    ...
    items: [
      ...
      "Связка AI и GitLab ускорила просмотр изменений в коде и заметно сократила время на составление чек-листов под релиз.",
  ...
  },
  {
    ...
    items: [
      ...
      "Платежные несоответствия: в отдельных сценариях после ошибки оплаты происходило зачисление средств, что ломало консистентность статусов.",
...
export const closingSummary: ClosingSummary = {
  headline: "QA-вклад видим, измерим и честно показывает неизвестные зоны.",
  body: `Всего выполнено ${totalProjects} проектов. По CSV-выгрузке подтверждено ${totalTestedTasksFromCsv} протестированных задач. Метрики в карточках проектов дополнительно показывают локальную детализацию по каждому кейсу (${totalTestedTasksFromCards} суммарно по карточкам).`,
  ctaLabel: "Смотреть полные QA-доказательства",
  evidenceLabel: "Смешанный доказательный контракт",
  evidenceNote:
    "Сочетает подтвержденные цифры из репозитория и явно помеченные плейсхолдеры для метрик без инструментирования.",
};
